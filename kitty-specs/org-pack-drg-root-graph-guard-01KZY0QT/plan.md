...
PLAN-ARCH-001's confirmed gap by construction — see IC-03, sole owner of both wraps). The project-layer
...
as the `drg/` case above, now also covered by IC-03's broadened design and worth the same
...
- **Risks**: `has_graph_files` only checks for the *presence* of a recognisably-named graph file, not its validity — a root-level `graph.yaml` that exists but is malformed still causes `has_root_graph=True`, and the subsequent load of that root content raises plain `DRGLoadError`. **As IC-01 lands on its own** (no dependencies, per Sequencing/depends-on above — IC-03 has not yet landed), this concern does **not** wrap that load: IC-01 owns only the existence-check/load-or-`None` guard, not any exception-wrapping behavior — that ownership belongs entirely to IC-03 (see IC-03's Purpose/Affected Surfaces). A malformed root graph therefore still propagates a plain `DRGLoadError` up through `_load_org_layer` into `load_validated_graph`, straight into the existing wide `except DRGLoadError` in `_load_action_doctrine_bundle`, exactly as it did before this mission. That existing catch collapses the **entire resolved action bundle to empty across all five doctrine kinds** (directive/tactic/styleguide/toolguide/procedure — built-in + org + project layers together, not merely the org layer's own contribution), because the propagating exception aborts `load_validated_graph` before the project-layer load or the built-in/project merge ever runs — so a perfectly valid, loadable sibling `drg/` fragment never even gets a chance to contribute either (a variant of #3384's own defect shape one directory over, and PLAN-ARCH-001's confirmed finding). This is the correct, unremediated interim state as IC-01 lands alone; **IC-03 is the sole owner of the fix**, wrapping both the root-level load and the `drg/`-level load independently so a malformed root graph raises `OrgDRGFragmentError` on its own and no longer takes a valid sibling `drg/` fragment down with it. See the paired Test Strategy rows below
  (`test_malformed_root_graph_with_valid_drg_sibling_raises_distinguishable_failure` and
  `test_malformed_root_graph_no_drg_directory_raises_distinguishable_failure`) that pin
  this as IC-03's closed behavior, not IC-01's.
...
### IC-03 — Malformed org-layer content (root graph or `drg/` fragment) surfaces as a real, distinguishable failure (FR-004; User Story 3 for the `drg/`-fragment case, the defect-class-closure rationale in `reviews/plan.ruling.md` for the root-level broadening; PLAN-ARCH-001/PLAN-FRESH-002 remediation option (b))
...
malformed `drg/` fragment (IC-03, sole owner of both wraps) is a considered plan-level design choice, not a scope
...
| Malformed root-level `graph.yaml` + valid `drg/` sibling: distinguishable failure, not silent zeroing | FR-001, FR-004; closes PLAN-ARCH-001/PLAN-FRESH-002's confirmed gap (remediation option (b)) | `test_malformed_root_graph_with_valid_drg_sibling_raises_distinguishable_failure` | tmp repo; root `graph.yaml` is malformed (invalid YAML or schema-invalid) **and** `drg/fixture.graph.yaml` alongside it is valid and declares a loadable, reachable node | Invoke the `charter context --action <a> --json` CLI entry point (not the internal function) and assert the JSON output has `result != "success"` **and** a non-empty `error` field — the same assertion shape as US3's malformed-`drg/`-fragment rows (`test_malformed_yaml_drg_fragment_raises_distinguishable_failure`), never a baseline-count assertion; a valid `drg/` fragment sitting next to a malformed root graph must not be silently taken down with it. | Revert IC-03's root-level wrap only (leave the `drg/`-level wrap in place): the malformed root graph's plain `DRGLoadError` is caught by the existing wide catch in `_load_action_doctrine_bundle` and silently collapses **the entire resolved action bundle to empty across all five doctrine kinds** (directive/tactic/styleguide/toolguide/procedure — built-in + org + project layers together, not merely the org layer's own contribution, because the propagating exception aborts `load_validated_graph` before the project-layer load or the built-in/project merge ever runs) — including the valid `drg/` content, which never gets a chance to contribute — to a zeroed-but-reported-success bundle (today's pre-fix shape) → CLI reports `"result": "success"` → assertion (`result != "success"`) fails → RED. |
| Malformed root-level `graph.yaml`, **no `drg/` directory/content at all** — standalone case, distinct from the combined root+`drg/` row above: distinguishable failure, not silent zeroing | FR-001, FR-004; the defect-class-closure rationale in `reviews/plan.ruling.md` (root-level broadening); closes PLAN-ARCH-001/PLAN-FRESH-002's confirmed gap (remediation option (b)) | `test_malformed_root_graph_no_drg_directory_raises_distinguishable_failure` | tmp repo; root `graph.yaml` is malformed (invalid YAML or schema-invalid); **no** `drg/` directory exists at all (not even an empty one) — the org pack's *only* content is the malformed root graph, with nothing under `drg/` to combine with it | Invoke the `charter context --action <a> --json` CLI entry point (not the internal function) and assert the JSON output has `result != "success"` **and** a non-empty `error` field — the same discriminating shape as the combined-case row above and US3's malformed-`drg/`-fragment rows, a shape User Story 2's genuinely-empty case never produces (that case reports `"result": "success"`). This row is deliberately its own test, not a restatement of the combined-case row: its fixture has no `drg/` content whatsoever, so it is the only row that goes RED against a conforming-but-incomplete implementation that fires the root-level wrap only when `drg/` also exists alongside it — closing the vacuity-by-empty-set hazard the ruling names. | Revert IC-03's root-level wrap entirely (imagine `_load_org_layer` never wraps the root-level load at all, only the `drg/`-level load): the malformed root graph's plain `DRGLoadError` is caught by the existing wide catch in `_load_action_doctrine_bundle` and silently collapses the entire resolved action bundle to empty across all five doctrine kinds (built-in + org + project layers together) — today's pre-fix shape, reopened — → CLI reports `"result": "success"` → assertion (`result != "success"`) fails → RED. |
...
## Silent Success Discipline (User Story 3 for the `drg/`-fragment case, the defect-class rationale in `reviews/plan.ruling.md` for the root-level broadening / FR-004)
